...
class ParkingManagementSystem:
    ...
    def load_parking_positions(self, reference_image=None):
        ...
        try:
            ...
            self.posList = load_parking_positions(self.config_dir, reference_image)
            self.total_spaces = len(self.posList)
            self.free_spaces = 0
            self.occupied_spaces = self.total_spaces

            # Add this line to scale positions to current dimensions
            self.scale_positions_to_current_dimensions()
            if hasattr(self, 'parking_visualizer') and hasattr(self, 'allocation_engine'):
                self.connect_parking_data()

            # Notify tabs of the updated positions
            if hasattr(self, 'detection_tab'):
                self.detection_tab.update_status_info(self.total_spaces, self.free_spaces,
    ...
    def scale_positions_to_current_dimensions(self):
        ...
        if self.current_reference_image in self.reference_dimensions:
            ...
            self.log_event(f"Scaling positions: reference {ref_width}x{ref_height}, "
                           f"current {self.image_width}x{self.image_height}, "
                           f"scale factors: ({width_scale:.3f}, {height_scale:.3f})")
            ...
            self.log_event(f"Warning: No reference dimensions for {self.current_reference_image}")
